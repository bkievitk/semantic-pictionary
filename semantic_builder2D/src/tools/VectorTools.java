...
public class VectorTools {
	...
	public static void showSize(double[][] a) {
		System.out.println(a.length + "x" + a[0].length);
	}
	
	public static double[][] toArray(Hashtable<String,double[]> hash) {
		double[][] entries = new double[hash.size()][];
		String[] list = hash.keySet().toArray(new String[0]);		
		for(int i=0;i<list.length;i++) {
			entries[i] = hash.get(list[i]);
		}	
		return entries;
	}
	
	public static void show(double[][] a, int limit) {
		for(int i=0;i<limit&&i<a.length;i++) {
			for(int j=0;j<limit&&j<a[0].length;j++) {
				
				String toPrint = String.format("%1.4f", a[i][j]);
	...
	public static double getCosine(double[] a, double[] b) {
		...
		double arcCos = dot(a,b) / sum;
		if(arcCos > .99999999) {
			return 1;
		}
		
		double ret = Math.acos(arcCos);
		
		if(Double.isNaN(ret)) {
			System.out.println("HERE - > " + sum / dot(a,b));
		}
	...
	public static double getArcCosine(double[] a, double[] b) {
		double sum = dist(a) * dist(b);
		if(sum == 0) {
			return -1;
		}
